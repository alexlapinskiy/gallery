...
//
//    $sql = "SELECT images.image_id, image_path, thumbnail_path, description, author_name, created_date FROM images
//LEFT JOIN users on images.user_id = users.user_id
//LIMIT " . $offset . ", " . IMAGE_COUNT;
//    //EDIT QUERIES PARAMETERS ALREADY!!!//
//
//
...
//{
//    $database = connect();
////    $sql = 'INSERT INTO images(image_id, image_path, thumbnail_path, description, author_name, created_at, user_id)
////VALUES(NULL, :image_path, :thumbnail_path, :description, :author_name, CURRENT_TIMESTAMP(), :user_id)';
//
//    $sql = 'INSERT INTO images(image_id, image_path, thumbnail_path, description, author_name, created_date, user_id)
//VALUES (NULL , :image_path, :thumbnail_path, :description, :author_name, CURRENT_TIMESTAMP (), :user_id)';
...
//        ]; //ADD OTHER FIELDS OR DELETE SOME FIELDS ALLREADY!!!!!//
...
//    $result = request($database, 'SELECT user_id FROM users WHERE user_login = :login AND user_pass = :pass', [':login' => $postUser, ':pass' => $pass]);
...
//    if (isLoggedIn()) {
//        $database = connect();
//        $image = request($database, "SELECT image_path, thumbnail_path FROM images WHERE image_id = :id", [':id' => $id]);
//        request($database, "DELETE FROM images WHERE image_id = :id", [':id' => $id]);
//        unlink($image->fetchColumn(0));
//        unlink($image->fetchColumn(1));
//        $_SESSION['messages'] = ['You have deleted image'];
//        return true;
//    } else {
//        $_SESSION['errors'] = ['You haven\'t access to delete images'];
//        return false;
//    }
//}
//
...
//    $database = connect();
//    if (request($database, 'INSERT INTO users(user_id, user_login, user_pass) VALUES (NULL, :login, :pass)', array(':login' => $login, ':pass' => $pass))) {
